...
class Demo_LLampuga():
    ...
    def initialization(self, weight_path ="mask_rcnn_coco.h5"):
        ...
        self.WEIGTHS_DIR = os.path.join(ROOT_DIR,"weights")
        ...
        self.INTERFERENCE_DIR = os.path.join(ROOT_DIR,"interference")
        self.create_directory(self.INTERFERENCE_DIR)
        print("INTERFERENCE_DIR PATH:" + self.INTERFERENCE_DIR)
        ...
        self.COCO_MODEL_PATH = os.path.join(ROOT_DIR, "weights/mask_rcnn_coco.h5")
    ...
    def create_model(self, mode = 'training', weigths_path =None):
        if mode == 'training':
            ...
            elif os.path.exists(self.WEIGTHS_DIR+ "/"+ weigths_path):
                model_path = self.WEIGTHS_DIR+ "/"+ weigths_path
            else:
    ...
    def create_augmentation(self, visualize_augm=False):
        ...
        if visualize_augm:
            ...
            image, image_meta, class_ids, bbox, mask = modellib.load_image_gt(self.dataset_train, self.config, image_id, use_mini_mask=False)
    ...
    def evaluation(self, number_of_images, confussion, directory):
        ...
        for image_id in image_ids:
            ...
            visualize.display_instances(image, r['rois'], r['masks'], r['class_ids'],self.dataset_val.class_names, r['scores'], ax=self.get_ax(size=20), captions=mi)
            ...
            DATOMAP = createMap(["ImageID", "Precision", "Recall", "Overlaps", "AP"],[image_id, np.mean(precisions),np.mean(recalls), np.mean(overlaps),AP], DATOMAP)

        pd.DataFrame(DATOMAP).to_csv(savedir+"/"+'metrics.csv', header=["ImageID", "Precision", "Recall", "Overlaps", "AP"], index=None)
        ...
        if confussion:
            ...
            confMatrix.plot_confusion_matrix_from_data(gt_tot, pred_tot, columns=self.dataset_val.class_names, annot=True,
                                                                          cmap="Oranges",
    ...
    def metadatos_extraction(self, directory = "OPMM_Subasta_2020-09-01/", visualize_img=True, save=False, llotja=False, new_path=None):
        ...
        if llotja:
            path= directory
            self.INTERFERENCE_DIR=new_path
        else:
            path = self.IMAGES_DIR+ "/"+ directory
                    ...
                    if isinstance(data, bytes):
                        ...
                        if (fao == 'DOL') and (ord1 == '1') and (caj == '01'):
                            ...
                            visualize.display_instances(image, r['rois'], r['masks'], r['class_ids'],
                                                        ...
                                                        captions=mi, visualize_img=visualize_img, save=save, output =new_directory+"/" + filename.split(".")[0] + '_interference.jpg')
                            ...
                            DAT = [filename,vta,ord1,emb,pes,caj, fao, np.mean(r['scores']), num_llamp]
    ...
    def organizeFile(self, date):
        ...
        self.create_directory(self.INTERFERENCE_DIR+ "/"+year)
        self.create_directory(self.INTERFERENCE_DIR+ "/"+year+"/"+month)
        self.create_directory(self.INTERFERENCE_DIR+ "/"+year + "/" + month+"/"+day)

        return self.INTERFERENCE_DIR+ "/"+year + "/" + month+"/"+day
...
class automatiscm():
    def initialize(self, mode, weight_path,training_dataset, validation_dataset, testing_dataset, visualize_images):
        self.demo = Demo_LLampuga()
        self.demo.initialization(weight_path =weight_path)
        self.demo.configuration()
        self.demo.initialize_dataset(mode = mode, training_dataset = training_dataset, validation_dataset=validation_dataset, testing_dataset = testing_dataset,  visualize_images=visualize_images)
        self.demo.create_model(mode=mode, weigths_path=weight_path)
    ...
    def evaluation(self, number_of_images, confussion, directory, weigths_path):
        self.demo.evaluation(number_of_images=number_of_images, confussion=confussion, directory=directory, weigths_path=weigths_path)

    def testing(self, directory = "OPMM_Subasta_2020-09-01/", visualize=True, save=False, llotja=False, new_path=None):
        self.demo.metadatos_extraction(directory=directory, visualize_img=visualize, save =save, llota=llotja, new_path=new_path)

    def parse_arguments(self):
        import argparse
        # Import arguments into the python script
        parser = argparse.ArgumentParser("IMEDEA: Project Deep Ecomar")
        parser.add_argument("--directory", type=str, default="OPMM_Subasta_2020-09-01/", help="Path to the directory of images to be tested")
        parser.add_argument('--not_display',  action="store_false", default=True, help='Introduce the argumnent to not display the processing of the images step by step')
        parser.add_argument('--save', action="store_true", default=False, help='Introduce the argument to save the processed images')
        parser.add_argument('--confussion', action="store_false", default=True, help='Whether to show the mattrix of confussion or not')
        parser.add_argument('--mode', type=str, default="testing", help='Introduce the aim of the execution: training, evaluating or testing')
        parser.add_argument('--weights_path', type=str, default="weights/mask_rcnn_llampuga_0068.h5", help='Introduce the path to the weigths')
        parser.add_argument('--epochs', type=int, default=100, help='Introduce the number of epochs for the training')
        parser.add_argument('--image_num', type=int, default=30, help='Introduce the number of images you want to process')
        parser.add_argument('--layers', type=str, default="heads", help='Introduce the layers tou want to train: either "all" or "heads"')
        parser.add_argument('--train_dataset', type=str, default="train.json", help='Introduce the path to the training dataset')
        parser.add_argument('--val_dataset', type=str, default="val.json", help='Introduce the path to the validation dataset')
        parser.add_argument('--test_dataset_images', type=str, default='', help='Introduce the path to the testing dataste image directory')
        parser.add_argument('--test_dataset_file', type=str, default='/test.json', help='Introduce the path to the testing dataset json file')
    ...
    def main(self):
        ...
        self.initialize(mode=mode, weight_path=args.weights_path, training_dataset=args.train_dataset, validation_dataset=args.val_dataset, testing_dataset=[args.test_dataset_images, args.test_dataset_file], visualize_images=False)
        ...
        if mode =="training":
            ...
            self.evaluation(directory=args.directory, confussion=args.confussion, number_of_images=args.image_num, weigths_path=args.weights_path)
